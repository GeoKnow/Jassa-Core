...
var PromiseUtils = {
    ...
    /**
     * Returns a function returning a promise that wraps a function returning promises.
     * Only the resolution of the oldest created promise will be resolved as long as this oldest promise is still running.
     * This is useful for simple scheduling of actions:
     * In the rex edit system, it can happen that a reset of the form state
     * is requested while a data fetching action is still running.
     * A reset should fetch that data anyway, however, if that action is already
     * running, we could chain to the already running promise:
     *
     * var reset = firstRequest(resetCore);
     *
     * var resetData = function() { reset().then(function() { ... } };
     *
     */
