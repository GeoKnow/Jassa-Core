...
var indexAccMap = function(state, sourceName, accMap) {
...
*/
    exec: function(decls, query) {
        ...
        var result = listServiceAcc.fetchItems(filterConcept, limit, offset).then(function(accEntries) {
            // Get the initial ids
            var rootIds = accs.map(function(accEntry) { // TODO We could use _.pluck here
                return accEntry.key;
            });
            ...
            state[sourceName] = map;
            var r = self.resolveRefs(decls, open, state);
            return [rootIds, r];

        }).spread(function(accs, r) {
    ...
    resolveRefs: Promise.method(function(decls, open, state) {
        ...
        var subPromises = sourceNames.map(function(sourceName) {
            ...
            var lookupService = self.createLookupService(sourceName);
            var nodes = set.entries();

            var subPromise = lookupService.lookup(nodes).then(function(accMap) {
                var accs = accMap.values();
                indexAccMap(state, sourceName, accMap);
                var refs = AccUtils.getRefs(accs);
