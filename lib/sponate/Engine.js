...
var Engine = {
    ...
    buildSlots : function(obj, result) {
        result = result || [];
        ...
        if (Array.isArray(obj)) {
            ...
            obj.forEach(function(item, index) {
                if(item._ref) {
                    var slot = new Slot(obj, index, item._ref);
                    result.push(slot);
                } else {
                    Engine.buildSlots(item, result);
                }
            });
    ...
    postProcess : function(state, sourceName, rootIds) {
        ...
        var slots = Engine.buildSlots(sourceToIdToObj);
