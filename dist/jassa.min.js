...
(function (global){
var Jassa=require("./index.js");"function"==typeof global.window.define&&global.window.define.amd?global.window.define("Jassa",function(){return Jassa}):global.window.Jassa=Jassa;
...
var Concept=require("../sparql/Concept"),Relation=require("../sparql/Relation"),NodeFactory=require("../rdf/NodeFactory"),Triple=require("../rdf/Triple"),NodeUtils=require("../rdf/NodeUtils"),ExprVar=require("../sparql/expr/ExprVar"),E_Equals=require("../sparql/expr/E_Equals"),NodeValue=require("../sparql/expr/NodeValue"),E_LogicalNot=require("../sparql/expr/E_LogicalNot"),E_OneOf=require("../sparql/expr/E_OneOf"),ElementGroup=require("../sparql/element/ElementGroup"),ElementFilter=require("../sparql/element/ElementFilter"),ElementTriplesBlock=require("../sparql/element/ElementTriplesBlock"),PatternUtils=require("../sparql/PatternUtils"),ElementUtils=require("../sparql/ElementUtils"),VarUtils=require("../sparql/VarUtils"),Step=require("./Step"),StepUtils=require("./StepUtils"),StepRelation=require("./StepRelation"),FacetConceptUtils={createConceptResources:function(e,t,r){var n=e.getBaseConcept(),a=e.getRootFacetNode(),l=e.getConstraintManager(),i=r?t:null,p=l.createElementsAndExprs(a,i),o=p.getElements(),s=p.getExprs(),u=a.forPath(t),c=u.getVar(),E=n.getElements(),g=u.getElements(),m=[];m.push.apply(m,g),m.push.apply(m,o),n.isSubjectConcept()?0===m.length&&(m=E):m.push.apply(m,E);var q=s.map(function(e){var t=new ElementFilter(e);return t});m.push.apply(m,q);var f=new ElementGroup(m).flatten(),v=new Concept(f,c);return v},createConceptFacets:function(e,t,r){var n=this.createRelationFacets(e,t,r),a=new Concept(n.getElement(),n.getSourceVar());return a},createRelationFacets:function(e,t,r,n){var a=e.getBaseConcept(),l=e.getRootFacetNode(),i=e.getConstraintManager(),p=null;n&&(p=new Step(n.getUri(),r));var o=null;p&&(o=t.copyAppendStep(p));var s,u=i.createElementsAndExprs(l,o),c=u.toElements(),E=l.forPath(t),g=E.getVar(),m=a.getElements();s=a.isSubjectConcept()?c:m.concat(c);var q=PatternUtils.getVarsMentioned(s),f=VarUtils.freshVar("p",q),v=VarUtils.freshVar("o",q),d=r?new Triple(g,f,v):d=new Triple(v,f,g);if(s.push(new ElementTriplesBlock([d])),p){var h=new ExprVar(f),V=new E_Equals(h,NodeValue.makeNode(n));s.push(new ElementFilter(V))}var w=E.getElements();s.push.apply(s,w);var F=new ElementGroup(s).flatten(),R=new Relation(F,f,v);return R},createRelationsFacetValues:function(e,t,r,n,a){var l=[];r=!!r;var i=(e.getBaseConcept(),e.getRootFacetNode()),p=e.getConstraintManager(),o=n.map(NodeUtils.getUri),s=i.forPath(t),u=p.getConstrainedSteps(t),c=u.filter(function(e){var t=e.isInverse()===r;if(!t)return!1;var n=o.indexOf(e.getPropertyName())>=0,l=a?!n:n;return l}),E=c.map(StepUtils.getPropertyName),g=[],m=[];n.forEach(function(e){E.indexOf(e.getUri())>=0?m.push(e):g.push(e)});var q=this.createStepRelations(s,c);l.push.apply(l,q);var f=this.createRelationFacets(t,r,!1),v=this.createElementFilterBindVar(f.getSourceVar(),g,!1);if(null!=v&&(f=new Relation(new ElementGroup([f.getElement(),v]),f.getSourcVar(),f.getTargetVar())),g.length>0){var d=new StepRelation(null,f);l.push(d)}return l},createElementFilterBindVar:function(e,t,r){var n=null;if(t.length>0){var a=new E_OneOf(new ExprVar(e),t);r&&(a=new E_LogicalNot(a)),n=new ElementFilter(a)}return n},createStepRelations:function(e,t,r){var n=this,a=r.map(function(r){var a=n.createStepRelation(e,t,r);return a});return a},createStepRelation:function(e,t,r){var n=r.getPropertyName(),a=NodeFactory.createUri(n),l=this.createRelationFacets(t,r.isInverse(),!1,a),i=new StepRelation(r,l);return i}};module.exports=FacetConceptUtils;
...
var Class=require("../ext/Class"),StepRelation=Class.create({initialize:function(t,e){this.step=t,this.relation=e},getStep:function(){return this.step},getRelation:function(){return this.relation},toString:function(){return this.step+": "+this.relation}});
...
var Class=require("../../ext/Class"),Concept=require("../../sparql/Concept"),ListServiceConcept=require("../../service/list_service/ListServiceConcept"),FacetService=require("./FacetService"),FacetUtils=require("./../FacetUtils"),FacetServiceSparql=Class.create(FacetService,{initialize:function(e,t){this.sparqlService=e,this.facetConfig=t},createListService:function(e,t){var i=FacetUtils.createRelationFacets(this.facetConfig,e,t),r=(new Concept(i.getElement(),i.getSourceVar()),new ListServiceConcept(this.sparqlService));return r}});module.exports=FacetServiceSparql;
...
var shared=require("./util/shared");module.exports=function(e,r){return shared.Promise=e,shared.ajax=r,{util:require("./util"),rdf:require("./rdf"),vocab:require("./vocab"),sparql:require("./sparql"),service:require("./service"),sponate:require("./sponate"),facete:require("./facete")}};
...
var Class=require("../ext/Class"),Relation=Class.create({initialize:function(e,t,r){this.element=e,this.sourceVar=t,this.targetVar=r},getElement:function(){return this.element},getSourceVar:function(){return this.sourceVar},getTargetVar:function(){return this.targetVar}});module.exports=Relation;
...
var Class=require("../../ext/Class"),ExprHelpers=require("../ExprHelpers"),E_Bound=Class.create({initialize:function(r){this.expr=r},copySubstitute:function(r){return new E_Bound(this.expr.copySubstitute(r))},getArgs:function(){return[this.expr]},copy:function(r){var e=ExprHelpers.newUnaryExpr(E_Bound,r);return e},toString:function(){return"bound("+this.expr+")"}});module.exports=E_Bound;
...
var ExprVar=require("../expr/ExprVar"),E_LangMatches=require("../expr/E_LangMatches"),E_LogicalOr=require("../expr/E_LogicalOr"),E_Lang=require("../expr/E_Lang"),E_Bound=require("../expr/E_Bound"),E_Regex=require("../expr/E_Regex"),E_Str=require("../expr/E_Str"),Concept=require("../Concept"),ElementGroup=require("../element/ElementGroup"),ElementOptional=require("../element/ElementOptional"),ElementFilter=require("../element/ElementFilter"),LabelUtils=require("../LabelUtils"),KeywordSearchUtils={createConceptRegex:function(e,r){var n=e.getElement(),t=e.getSourceVar(),E=e.getTargetVar(),l=new ExprVar(t),i=new ExprVar(E),a=new E_Regex(new E_Str(i),r,"i"),o=new E_LogicalOr(new E_Regex(new E_Str(l),r,"i"),new E_Bound(i)),p=new ElementGroup([new ElementOptional(new ElementGroup([n,new ElementFilter(a)])),new ElementFilter(o)]),u=new Concept(p,t);return u}};module.exports=KeywordSearchUtils;
